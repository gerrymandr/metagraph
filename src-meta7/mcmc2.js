...
for (let n = 0; n < square7sColumn; n++) {
  ...
  var rows = grd3
    ...
    .attr("district", function(d, i) {
      return cur_plan_str3[7 * n + i];
    })
    .style("fill", function(d, i) {
      return distfills[cur_plan_str3[7 * n + i] - 1];
    })
    .style("stroke", "#555")
    .style("stroke-width", 1);
}
  ...
  grd3.selectAll("rect").each(function(d) {
    ...
    if (nm != null) {
      d3.select(this).style("fill", function() {
        return distfills[
          cur_plan_str3[7 * parseInt(nm[2] - 1) + parseInt(nm[3]) - 1] - 1
        ];
      });
    }
  });
